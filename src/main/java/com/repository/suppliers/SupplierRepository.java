...
public interface SupplierRepository extends JpaRepository<BuyerEntity, String>, JpaSpecificationExecutor<BuyerEntity> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update buyer set buyerName = ?2, contactInformation = ?3, remarks = ?4, productionCategory = ?5, purchasingCategories = ?6, legalPerson = ?7, contact = ?8, sex = ?9, post = ?10, mail = ?11 where buyerNumber = ?1", nativeQuery = true)
